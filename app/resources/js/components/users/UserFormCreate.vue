<template>
    <div class="modal fade" id="modalUserFormCreate" tabindex="-1" role="dialog">
...
<script>
    ...
    export default {
        ...
        mounted() {
            $('#modalUserForm').on('hidden.bs.modal', () => {
                this.modalClose();
            });
            $('#modalUserForm').on('shown.bs.modal', () => {
                this.modalOpen();
            });
        },
        methods: {
            ...
            send() {
                this.process = true;
                window.axios.post('/users', JSON.stringify(this.user)).then(() => {
                    this.success = true;
                    this.$v.$reset();
                    this.user = {
                        id: null,
